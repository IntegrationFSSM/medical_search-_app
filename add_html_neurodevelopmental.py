...
def add_html_paths_neurodevelopmental():
    """Add html_page field to Neurodevelopmental JSON files in Gemini and OpenAI folders."""
    
    source_base = Path("Embedding/Neurodevelopmental_Disorders_out")
    target_bases = [
        Path("Embedding_Gemini_3072/Neurodevelopmental_Disorders_out"),
        Path("Embedding_OpenAI_3072/Neurodevelopmental_Disorders_out")
    ]
    
    print("=" * 80)
    print("AJOUT DES CHEMINS HTML - NEURODEVELOPMENTAL DISORDERS")
    print("=" * 80)
    print()
        ...
        for json_file in sorted(subdir.glob("*.json")):
            try:
                ...
                if 'html_page' not in source_data:
                    ...
                    json_stem = json_file.stem  # Ex: SubSection1.1_Intellectual_Developmental_Disorder
                    
                    # Trouver le fichier HTML dans le même sous-dossier
                    ...
                    relative_html_path = f"Neurodevelopmental_Disorders_out/{subdir.name}/{html_file.name}"
...
if __name__ == "__main__":
    add_html_paths_neurodevelopmental()
