...
def add_html_paths_sleep_wake():
    """Add html_page field to Sleep_Wake_Disorders JSON files in Gemini and OpenAI folders."""
    
    source_base = Path("Embedding/Sleep_Wake_Disorders_out")
    target_bases = [
        Path("Embedding_Gemini_3072/Sleep_Wake_Disorders_out"),
        Path("Embedding_OpenAI_3072/Sleep_Wake_Disorders_out")
    ]
    
    print("=" * 80)
    print("AJOUT DES CHEMINS HTML - SLEEP-WAKE DISORDERS")
    print("=" * 80)
    print()
        ...
        for json_file in sorted(subdir.glob("*.json")):
            try:
                ...
                if 'html_page' not in source_data:
                    ...
                    relative_html_path = f"Sleep_Wake_Disorders_out/{subdir.name}/{html_file.name}"
